...
const etapas = [
  ...
  // 🔹 ETAPA 21 — Adicionada: Análise de Viabilidade Celular
...
/* ═══════════════════════════════════════════════════════════
   ETAPA 21 — INTERFACE DE MICROSCÓPIO DE VIABILIDADE CELULAR
...
const MicroscopioViabilidade = ({ onConcluir }) => {
  ...
  const [celulas] = React.useState(() => {
    ...
    for (let i = 0; i < 40; i++) {
      arr.push({ tipo: 'viva', x: rand(GX + 8, GX + GW - 8), y: rand(GY + 8, GY + GH - 8), r: rand(5, 9), opac: rand(0.55, 0.85) });
    }
    for (let i = 0; i < 18; i++) {
      arr.push({ tipo: 'morta', x: rand(GX + 8, GX + GW - 8), y: rand(GY + 8, GY + GH - 8), r: rand(4.5, 8), opac: rand(0.70, 0.92) });
    }
    for (let i = 0; i < 8; i++) {
      const x = rand(GX + 14, GX + GW - 14);
      const y = rand(GY + 14, GY + GH - 14);
      const r = rand(6, 10);
  ...
  const renderNeubauer = () => {
    ...
    // Linhas externas triplas (característica do hemocitômetro real)
    [OY, OY + GH].forEach(y => {
      [-1.4, 0, 1.4].forEach(dy => {
        linhas.push(<line key={`th${y}${dy}`} x1={OX - 2} y1={y + dy} x2={OX + GW + 2} y2={y + dy} stroke="rgba(0,40,140,0.90)" strokeWidth="0.9" />);
      });
    });
    [OX, OX + GW].forEach(x => {
      [-1.4, 0, 1.4].forEach(dx => {
        linhas.push(<line key={`tv${x}${dx}`} x1={x + dx} y1={OY - 2} x2={x + dx} y2={OY + GH + 2} stroke="rgba(0,40,140,0.90)" strokeWidth="0.9" />);
      });
    });

    // Grade 3×3 de quadrantes
    for (let i = 1; i <= 2; i++) {
    ...
    // Sub-grade do quadrante central
    const micro = sq / 4;
    ...
    // Quadrantes dos cantos: grade 4×4 interna
    ...
    [
      ...
      { label: 'B', x: OX + Q * 2 + 4, y: OY + Q * 0 + 13 },
      { label: 'C', x: OX + Q * 1 + 4, y: OY + Q * 1 + 13 },
      { label: 'D', x: OX + Q * 0 + 4, y: OY + Q * 2 + 13 },
      { label: 'E', x: OX + Q * 2 + 4, y: OY + Q * 2 + 13 },
    ].forEach(({ label, x, y }) => {
      linhas.push(<text key={`lbl${label}`} x={x} y={y} fontSize="9" fontWeight="700" fill="rgba(0,50,160,0.55)" fontFamily="monospace">{label}</text>);
    });
        ...
        <div style={{ display: 'flex', gap: 0 }}>
          ...
          <div style={{ flex: 1, padding: '18px 16px 18px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <div style={{ width: 340, height: 340, borderRadius: '50%', background: 'radial-gradient(circle at 38% 32%, #1a2e1a 0%, #0a150a 60%, #030808 100%)', border: '8px solid #1a2035', boxShadow: '0 0 0 4px #0d1525, 0 0 40px rgba(0,0,0,0.9), inset 0 0 60px rgba(0,0,0,0.7)', overflow: 'hidden', position: 'relative', cursor: 'crosshair' }}>
                ...
                <div style={{ width: '100%', height: '100%', overflow: 'hidden', position: 'relative', filter: focado ? 'blur(0px)' : focando ? 'blur(2.5px)' : 'blur(5px)', transition: 'filter 1.2s ease' }}>
                  <div style={{ transform: `scale(${zoom}) translate(${panX / zoom}px, ${panY / zoom}px)`, transformOrigin: 'center center', transition: 'transform 0.22s ease', width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'center', alignItems: 'center' }}>
                ...
                <div style={{ width: 90, height: 6, background: 'rgba(255,255,255,0.08)', borderRadius: 4, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${((zoom - 0.6) / 3.4) * 100}%`, background: 'linear-gradient(90deg,#0ea5e9,#6366f1)', borderRadius: 4, transition: 'width 0.2s' }} />
                </div>
          ...
          <div style={{ width: 310, background: 'rgba(0,0,0,0.28)', borderLeft: '1px solid rgba(0,200,255,0.12)', borderRadius: '0 0 22px 0', padding: '18px 16px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              ...
              {(contVivas || contMortas) && (
                <div style={{ background: 'rgba(0,229,255,0.07)', border: '1px solid rgba(0,229,255,0.22)', borderRadius: 10, padding: '10px 12px', marginTop: 8 }}>
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#0a0f1a,#0d1f35,#0a1628)', padding: 16, fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      <div style={{ maxWidth: 1380, margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 28, marginBottom: 36, alignItems: 'end' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}
                ...
                <div
                  ...
                  style={{ cursor: etapaAtual === 20 || etapaAtual === 21 ? 'pointer' : 'default', background: 'rgba(0,0,0,0.22)', borderRadius: 14, padding: '8px 12px' }}
        ...
        <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(210px, 1fr))', gap: 8 }}>
          {etapas.map((et, i) => (
            <div key={et.id} style={{
              background: i < etapaAtual ? 'rgba(34,197,94,0.13)' : i === etapaAtual ? 'rgba(14,165,233,0.16)' : 'rgba(255,255,255,0.04)',
              border: `1px solid ${i < etapaAtual ? 'rgba(34,197,94,0.38)' : i === etapaAtual ? 'rgba(14,165,233,0.5)' : 'rgba(255,255,255,0.07)'}`,
              borderRadius: 10, padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 8,
            }}>
              <span style={{ fontSize: 14 }}>{i < etapaAtual ? '✅' : i === etapaAtual ? '🔬' : '⬜'}</span>
              <span style={{ fontSize: 9.5, color: i < etapaAtual ? '#4ade80' : i === etapaAtual ? '#7dd3fc' : '#64748b', fontWeight: i === etapaAtual ? 700 : 400 }}>
                {i + 1}. {et.titulo}
              </span>
            </div>
          ))}
