...
/**
 * useSimuladorDragCondutividadeAlcool — drag & drop para Condutividade em Álcool.
 *
 * Drag steps: 0 (amostra→bequer), 3 (eletrodo→bequer).
 * Clique (1, 2, 5) e timers (4, 6) tratados no componente.
 */
export const useSimuladorDragCondutividadeAlcool = (etapas, {
  ...
  const handleDrop = (alvo, e) => {
    ...
    if (itemDrop === 'amostra' && alvo === 'bequer' && etapaAtual === 0) {
      ...
      setNivelBequer(60);
      setCorBequer('rgba(240,230,170,0.78)'); // álcool — levemente âmbar
