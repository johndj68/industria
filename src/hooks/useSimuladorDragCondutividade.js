...
/**
 * useSimuladorDragCondutividade — drag & drop para Condutividade e STD/TDS.
 *
 * Drag steps: 0 (amostra→bequer), 3 (eletrodo→bequer).
 * Clique (1, 2, 5) e timers (4, 6) tratados no componente.
 */
export const useSimuladorDragCondutividade = (etapas, {
  ...
  const handleDrop = (alvo, e) => {
    ...
    // E3 — ELETRODO → BÉQUER (imergir bulbo na amostra)
    if (itemDrop === 'eletrodo' && alvo === 'bequer' && etapaAtual === 3) {
      setItemSegurado(null);
      setEletrodoImergido(true);
      setEstabilizando(true); // useEffect no componente cuida do timer
      celebrarAcerto();
      proximaEtapa(); // → step 4 (auto stabilization timer)
      return;
    }
  };
